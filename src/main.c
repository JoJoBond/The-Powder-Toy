...
void stamp_gen_thumb(int i)
{
	...
	if (data)
	{
		stamps[i].thumb = Graphics_PrerenderSave(data, size, &(stamps[i].thumb_w), &(stamps[i].thumb_h));
...
static PyObject* emb_draw_pixel(PyObject *self, PyObject *args)
{
	...
	if (sdl_scrn!=NULL)
	{
		Renderer_BlendPixel(x,y,r,g,b,a);
		return Py_BuildValue("i",1);
	}
	return Py_BuildValue("i",-1);
...
static PyObject* emb_draw_text(PyObject *self, PyObject *args)
{
	...
	if (sdl_scrn!=NULL)
	{
		Graphics_RenderText(x,y,txt,r,g,b,a);
		return Py_BuildValue("i",1);
	}
	return Py_BuildValue("i",-1);
}
...
static PyObject* emb_draw_rect(PyObject *self, PyObject *args)
{
	...
	if (sdl_scrn!=NULL)
	{
		Renderer_DrawRectangle(x,y,w,h,r,g,b,a);
		//fillrect
		return Py_BuildValue("i",1);
	}
	return Py_BuildValue("i",-1);
...
static PyObject* emb_draw_fillrect(PyObject *self, PyObject *args)
{
	...
	if (sdl_scrn!=NULL)
	{
		Renderer_FillRectangle(x,y,w,h,r,g,b,a);
		//fillrect
		return Py_BuildValue("i",1);
	}
	return Py_BuildValue("i",-1);
...
static PyObject* emb_get_width(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "s:get_width",&txt))
		return NULL;
	return Py_BuildValue("i",GetTextWidth(txt));
}
...
#ifdef PYCONSOLE
int process_command(char *console,char *console_error,PyObject *pfunc) {
...
#endif
int process_command_old(char *console,char *console_error) {
	int y,x,nx,ny,i,j,k,m;
	float f;
	int do_next = 1;
	char xcoord[10] = "";
						...
						if (fileread[i] == '\n')
						{
							...
							if (do_next)
							{
								if (strcmp(pch,"else")==0)
									...
									do_next = process_command_old(pch, console_error);
							}
...
int main(int argc, char *argv[])
{
	...
	if(load_data && load_size){
		...
		for(i=0; i<30; i++){
			Renderer_PrepareScreen();
			update_particles();
			Graphics_RenderWalls();
			Graphics_RenderParticles();
			Renderer_DrawFire();
		}
		
		Graphics_RenderSigns();
		
		if(parsestate>0){
			//return 0;
			info_box("Save file invalid or from newer version");
		}
...
int main(int argc, char *argv[])
{
	...
	//char console_error[255] = "";
	int i, j, bq, do_check=0, do_s_check=0, old_version=0, http_ret=0,http_s_ret=0, major, minor, old_ver_len;
#ifdef INTERNAL
	int vs = 0;
#endif
...
//TODO: Move out version stuff
	...
	if (is_beta)
	{
		old_ver_len = GetTextWidth((char*)old_ver_msg_beta);
	}
	else
	{
		old_ver_len = GetTextWidth((char*)old_ver_msg);
	}
#else
	old_ver_len = GetTextWidth((char*)old_ver_msg);
#endif
	...
	while (!sdl_poll()) //the main loop
	{
		...
		update_particles(); //update everything
		Graphics_RenderWalls();
		Graphics_RenderParticles(); //draw particles
		
		if (cmode==CM_PERS)
			Renderer_GrabPersistent();
		...
		Renderer_ClearMenu();//clear menu areas
		...
		if (http_session_check)
		{
			if (!do_s_check && http_async_req_status(http_session_check))
			{
				...
				Renderer_ClearRectangle(XRES-125+BARSIZE/*385*/, YRES+(MENUSIZE-16), 91, 14);
				...
				if (svf_login)
					Graphics_RenderText(XRES-104+BARSIZE/*406*/, YRES+(MENUSIZE-12), svf_user, 255, 255, 255, 175-(username_flash*5));
				else
					Graphics_RenderText(XRES-104+BARSIZE/*406*/, YRES+(MENUSIZE-12), "[checking]", 255, 255, 255, 255);
			}
		...
		if (sys_shortcuts==1)//all shortcuts can be disabled by python scripts
		{
			...
			{
				if(confirm_ui("Install Powder Toy", "You are about to install The Powder Toy", "Install"))
				{
					if(register_extension())
					{
						info_ui("Install success", "Powder Toy has been installed!");
					}
					else
					{
						error_ui(0, "Install failed - You may not have permission or you may be on a platform that does not support installation");
					}
				}
			}
			...
			if ((sdl_key=='l' || sdl_key=='k') && stamps[0].name[0])
			{
				...
				if (load_data)
				{
					load_img = Graphics_PrerenderSave(load_data, load_size, &load_w, &load_h);
					if (load_img)
						load_mode = 1;
					else
						free(load_data);
		...
		{
			Graphics_RenderMenu(i, active_menu);
		}
		...
		menu_ui_v3(active_menu, &sl, &sr, b, bq, x, y); //draw the elements in the current menu
		...
		else if (b)//there is a click
		{
					...
					if (x>=37 && x<=187 && svf_login)
					{
						if (!svf_open || !svf_own || x>51)
						{
							if (save_name_ui()) {
								execute_save();
								if (svf_id[0]) {
									copytext_ui("Save ID", "Saved successfully!", svf_id);
								}
							}
						}
						else
			...
			else if (y<YRES)// mouse is in playing field
			{
					...
					if (!bq)
						for (signi=0; signi<MAXSIGNS; signi++)
							if (sregexp(signs[signi].text, "^{c:[0-9]*|.*}$")==0)
							{
								...
								Interface_GetSignPosition(signi, &signx, &signy, &signw, &signh);
				...
				else if (lb)//lb means you are holding mouse down
				{
					...
					{
						Renderer_XORLine(lx, ly, lx, y);
						Renderer_XORLine(lx, y, x, y);
						Renderer_XORLine(x, y, x, ly);
						Renderer_XORLine(x, ly, lx, ly);
					}
		...
		{
			Renderer_DrawImage(load_img, load_x, load_y, load_w, load_h, 128);
			Renderer_XORRectangle(load_x, load_y, load_w, load_h);
		}

		if (save_mode)//draw dotted lines for selection
		{
			Renderer_XORRectangle(save_x*CELL, save_y*CELL, save_w*CELL, save_h*CELL);
		...
		{
			Graphics_RenderCursor(mx/sdl_scale, my/sdl_scale, su, bsx, bsy);
			mousex = mx/sdl_scale;
			mousey = my/sdl_scale;
		}
		...
		if (da)
			...
			{
			case 256:
				Graphics_RenderText(16, YRES-24, "Add simulation tags.", 255, 255, 255, da*5);
				break;
			case 257:
				Graphics_RenderText(16, YRES-24, "Add and remove simulation tags.", 255, 255, 255, da*5);
				break;
			case 258:
				Graphics_RenderText(16, YRES-24, "Save the simulation under the current name.", 255, 255, 255, da*5);
				break;
			case 259:
				Graphics_RenderText(16, YRES-24, "Save the simulation under a new name.", 255, 255, 255, da*5);
				break;
			case 260:
				Graphics_RenderText(16, YRES-24, "Sign into the Simulation Server.", 255, 255, 255, da*5);
				break;
			case 261:
				Graphics_RenderText(16, YRES-24, "Sign into the Simulation Server under a new name.", 255, 255, 255, da*5);
				break;
			case 262:
				Graphics_RenderText(16, YRES-24, "Find & open a simulation", 255, 255, 255, da*5);
				break;
			case 263:
				Graphics_RenderText(16, YRES-24, "Pause the simulation", 255, 255, 255, da*5);
				break;
			case 264:
				Graphics_RenderText(16, YRES-24, "Resume the simulation", 255, 255, 255, da*5);
				break;
			case 265:
				Graphics_RenderText(16, YRES-24, "Reload the simulation", 255, 255, 255, da*5);
				break;
			case 266:
				Graphics_RenderText(16, YRES-24, "Erase all particles and walls", 255, 255, 255, da*5);
				break;
			case 267:
				Graphics_RenderText(16, YRES-24, "Change display mode", 255, 255, 255, da*5);
				break;
			case 268:
				Graphics_RenderText(16, YRES-24, "Annuit C\245ptis", 255, 255, 255, da*5);
				break;
			case 269:
				Graphics_RenderText(16, YRES-24, "Click-and-drag to specify a rectangle to copy (right click = cancel).", 255, 216, 32, da*5);
				break;
			case 270:
				Graphics_RenderText(16, YRES-24, "Enable or disable compatibility mode (disables heat simulation).", 255, 255, 255, da*5);
				break;
			case 271:
				Graphics_RenderText(16, YRES-24, "You're a moderator", 255, 255, 255, da*5);
				break;
			case 272:
				Graphics_RenderText(16, YRES-24, "Like/Dislike this save.", 255, 255, 255, da*5);
				break;
			case 273:
				Graphics_RenderText(16, YRES-24, "You like this.", 255, 255, 255, da*5);
				break;
			case 274:
				Graphics_RenderText(16, YRES-24, "You dislike this.", 255, 255, 255, da*5);
				break;
			case 275:
				Graphics_RenderText(16, YRES-24, "You cannot vote on your own save.", 255, 255, 255, da*5);
				break;
			default:
				Graphics_RenderText(16, YRES-24, (char *)ptypes[db].descs, 255, 255, 255, da*5);
			}
		if (itc)//message in the middle of the screen, such as view mode changes
		{
			itc--;
			Graphics_RenderText((XRES-GetTextWidth(itc_msg))/2, ((YRES/2)-10), itc_msg, 255, 255, 255, itc>51?255:itc*5);
		}
		if (it)//intro message
		{
			it--;
			Graphics_RenderText(16, 20, it_msg, 255, 255, 255, it>51?255:it*5);
		}

		if (old_version)
		{
			Renderer_ClearRectangle(XRES-21-old_ver_len, YRES-24, old_ver_len+9, 17);
#ifdef BETA
			if (is_beta)
			{
				Graphics_RenderText(XRES-16-old_ver_len, YRES-19, old_ver_msg_beta, 255, 216, 32, 255);
			}
			else
			{
				Graphics_RenderText(XRES-16-old_ver_len, YRES-19, old_ver_msg, 255, 216, 32, 255);
			}
#else
			Graphics_RenderText(XRES-16-old_ver_len, YRES-19, old_ver_msg, 255, 216, 32, 255);
#endif
			Renderer_DrawRectangle(XRES-19-old_ver_len, YRES-22, old_ver_len+5, 13, 255, 216, 32, 255);
		}
		...
		if (hud_enable)
		{
			...
			if (sdl_zoom_trig||zoom_en)
			{
				if (zoom_x<XRES/2)
				{
					Renderer_FillRectangle(XRES-20-GetTextWidth(heattext), 266, GetTextWidth(heattext)+8, 15, 0, 0, 0, 140);
					Graphics_RenderText(XRES-16-GetTextWidth(heattext), 270, heattext, 255, 255, 255, 200);
					if (DEBUG_MODE)
					{
						Renderer_FillRectangle(XRES-20-GetTextWidth(coordtext), 280, GetTextWidth(coordtext)+8, 13, 0, 0, 0, 140);
						Graphics_RenderText(XRES-16-GetTextWidth(coordtext), 282, coordtext, 255, 255, 255, 200);
					}
					if (wavelength_gfx)
						draw_wavelengths(XRES-20-GetTextWidth(heattext),265,2,wavelength_gfx);
				}
				else
				{
					Renderer_FillRectangle(12, 266, GetTextWidth(heattext)+8, 15, 0, 0, 0, 140);
					Graphics_RenderText(16, 270, heattext, 255, 255, 255, 200);
					if (DEBUG_MODE)
					{
						Renderer_FillRectangle(12, 280, GetTextWidth(coordtext)+8, 13, 0, 0, 0, 140);
						Graphics_RenderText(16, 282, coordtext, 255, 255, 255, 200);
					}
					if (wavelength_gfx)
						draw_wavelengths(12,265,2,wavelength_gfx);
				}
			}
			else
			{
				Renderer_FillRectangle(XRES-20-GetTextWidth(heattext), 12, GetTextWidth(heattext)+8, 15, 0, 0, 0, 140);
				Graphics_RenderText(XRES-16-GetTextWidth(heattext), 16, heattext, 255, 255, 255, 200);
				if (DEBUG_MODE)
				{
					Renderer_FillRectangle(XRES-20-GetTextWidth(coordtext), 26, GetTextWidth(coordtext)+8, 11, 0, 0, 0, 140);
					Graphics_RenderText(XRES-16-GetTextWidth(coordtext), 27, coordtext, 255, 255, 255, 200);
				}
				if (wavelength_gfx)
					draw_wavelengths(XRES-20-GetTextWidth(heattext),11,2,wavelength_gfx);
			}
			wavelength_gfx = 0;
		...
		if (console_mode)
		{
			...
			if (pyready==1 && pygood==1)
			{
				...
				sys_pause = 1;
				console = console_ui(console_error,console_more);
				console = mystrdup(console);
				strcpy(console_error,"");
				if (process_command(console, console_error,pfunc)==-1)
				{
					free(console);
					break;
				}
			...
			else
			{
				...
				console = mystrdup(console);
				strcpy(console_error,"");
				if (process_command_old(console, console_error)==-1)
				{
					free(console);
					break;
				}
